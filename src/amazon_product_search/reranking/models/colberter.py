...
class ColBERTer(nn.Module):
    ...
    def forward(self, query: dict[str, torch.Tensor], doc: dict[str, Any]):
    ...
    def encode_query(self, query: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    def encode_doc(self, doc: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    def encode_shared(self, tokens: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    def compute_cls_score(self, query_cls_vec: torch.Tensor, doc_cls_vec: torch.Tensor) -> torch.Tensor:
    ...
    def compute_term_score(
        ...
        exact_scoring_mask: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
